    ...
    <ul>
      <li>
        <div class="list-group testch">
          ...
          <ul>
            <?php
              $query = "SELECT DISTINCT(location) FROM room_data ORDER BY location ASC";
      ...
      <li>
        <div class="list-group testch">
          <label for="btn1">Floor</label>
          <input class="chk" type="checkbox" id="btn1">
          <ul>
            <?php
              $query = "SELECT DISTINCT(floor) FROM room_data ORDER BY floor ASC";
              $statement = $connect->prepare($query);
              $statement->execute();
              $result = $statement->fetchAll();
              foreach($result as $row)
              ...
              ?>
                <li>
                  <div class="list-group-item checkbox">
                    <input type="checkbox" class="filter_all floor" value="<?php echo $row['floor']; ?>">
                    <?php echo $row['floor']; ?> 
                  </div>
                </li>
              <?php
      ...
      <li><!-- rooms per floor filter -->
        <div class="list-group testch">
          <label for="btn2">Number of room</label>
          <input class="chk" type="checkbox" id="btn2">
          <ul>
            <?php
              $query = "SELECT DISTINCT(room) FROM room_data ORDER BY room ASC";
              $statement = $connect->prepare($query);
              $statement->execute();
              $result = $statement->fetchAll();
              foreach($result as $row)
      ...
      <li><!-- pet filter -->
        <div class="list-group testch">
          <label for="btn3">Pet</label>
          <input class="chk" type="checkbox" id="btn3">
          <ul>
            <?php
              $query = "
              SELECT DISTINCT(pet) FROM flat_data ORDER BY pet ASC";
              $statement = $connect->prepare($query);
              $statement->execute();
              $result = $statement->fetchAll();
              foreach($result as $row)
      ...
      <li><!-- parking filter -->
        <div class="list-group testch">
          <label for="btn4">Parking</label>
          <input class="chk" type="checkbox" id="btn4">
          <ul>
            <?php
              $query = "
              SELECT DISTINCT(parking) FROM room_data ORDER BY parking ASC";
              $statement = $connect->prepare($query);
              $statement->execute();
              $result = $statement->fetchAll();
              foreach($result as $row)
      ...
      <li><!-- purpose filter -->
        <div class="list-group testch">
          <label for="btn5">Purpose</label>
          <input class="chk" type="checkbox" id="btn5">
          <ul>
            <?php
              $query = "
              SELECT DISTINCT(purpose) FROM room_data ORDER BY purpose ASC";
              $statement = $connect->prepare($query);
              $statement->execute();
              $result = $statement->fetchAll();
              foreach($result as $row)
      ...
      <li><!-- rent filter -->
        <div class="list-group testch rt-flt">
          <label for="btn6">Rent</label>
          <input class="chk" type="checkbox" id="btn6">
          <ul>
            <li>
              <div class="list-group-item">
                <input type="hidden" id="min_price_hide" value="1000" />
                <input type="hidden" id="max_price_hide" value="30000" />
                <p id="price_show">Rs.1000 - Rs.30000</p>
        ...
        <div class="room-card">
          ...
          <a href="singleflat.php">
            ...
            <div class="flat-brief">
              <p><span class="room-list-tag">Room</span></p>
...
<script>
        $(document).ready(function() {
            ...
            function filter_data() {
                ...
                $.ajax({
                    url: "fetchdata/fetch_roomdata.php",
                    method: "POST",
                    data: {
                        action: action,
